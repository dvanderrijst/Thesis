...
//                    }
        ...
        for (int k : K) {
            for (int i0 : I0) {
                IloNumVar var = cplex.intVar(1,m*N+1,"t("+k+","+i0+")"); //m*N+1 seems like a reasonable upperbound.
                t[k][i0] = var;
            }
        }
    }
        ...
        for (int i0 : I0) {
            for (int i1 : I1) {
                for (int i2 : I2) {
                    ...
                    //constraint 9e en 9f
                    if(i2!=0 & i2!=M)                {cplex.addLe(cplex.diff(x[i0][i1][i2][1],   z[1][i0][i1]), 0.0,"9e"+i0+","+i1+","+i2+",k="+1+"a=1");
                                                      cplex.addLe(cplex.sum (x[i0][i1][i2][1],   z[2][i0][i2]), 1.0,"9f"+i0+","+i1+","+i2+",k="+1+"a=1");}

                    if(i1!=0 & i1!=M)                {cplex.addLe(cplex.diff(x[i0][i1][i2][2],   z[2][i0][i2]), 0.0,"9e"+i0+","+i1+","+i2+",k="+2+"a=1");
    ...
    private void setConstraint9hi() throws IloException{
        for (int i0 : I0){
            for (int j0: I0 ) {
                for ( int k : K){
                    if(j0 < i0){
                        IloNumExpr sumL = cplex.sum(t[k][i0], cplex.prod(j0, y[k][j0]), cplex.prod(m*N, y[k][j0]));
                        int sumR = m*N + i0;
    ...
    private void setConstraint9jk() throws IloException{
        for (int i0 : I0){
            for (int k : K){
                ...
                //constraint 9j
                for(int ik : I1){
                        IloNumExpr diff = cplex.diff(z[k][i0][ik], y[k][i0]);
                ...
                for(int ik: I1){
                    for(int jk : I1){
                        if (jk > ik){
                            IloNumExpr diff = cplex.diff(z[k][i0][ik], z[k][i0][jk]);
