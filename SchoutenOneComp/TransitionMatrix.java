...
public class TransitionMatrix {
    ...
    public TransitionMatrix(Instance instance, Policy policy) {
        this.i = instance;
        this.policy = policy;
     ...
     * @return
     */
    public double[][] getPiPowerMatrix(int x) throws IllegalArgumentException {
    ...
    private double[][] setupPi() {
        double[][] pi_i0_i1_j0_j1_policy = new double[i.I0.length * i.I1.length][i.I0.length * i.I1.length]; //we have action state {0,1}.

        for (int i0 = 0; i0 < i.I0.length; i0++) {
            for (int i1 = 0; i1 < i.I1.length; i1++) {
                for (int j0 = 0; j0 < i.I0.length; j0++) {
                    for (int j1 = 0; j1 < i.I1.length; j1++) {
                        int a = policy.get(i0,i1);

                        double prob ;
                        if(a==-1){
                            prob = 0;
                        } else{
                            prob = i.piOneDim(i0,i1,j0,j1,a);
                        }
                        pi_i0_i1_j0_j1_policy[i0 + i1 * i.I0.length][j0 + j1 * i.I0.length] = prob;
                    }
                }
            }
        }
